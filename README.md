...
**PLAY "THE CASTING"** is the headline demo: a stage-ready cut of a *real* production run (3 July 2026). The AI, as the **Series Producer**, hires a **Trend Scout** to find what's genuinely hot, a **Concept Architect** to turn the strongest seed into a returnable format, and a **Sense-Checker** to pressure-test it adversarially — looping Architect ⇄ Sense-Checker across **three rounds** until the concept survives — then a **Deck Producer** to ship a 3-page deck. The real run used the Workflow orchestrator (its team lived in separate `subagents/workflows/*` files the theatre couldn't fold in), so `scripts/make_casting_demo.py` re-expresses it as `Task` spawns the adapter renders on stage. It ends on a **finale reveal**: the three real Magnific cover images and a button to open the finished deck (`static/demo/the-casting/pitch-deck.html`). Regenerate the transcript with `python scripts/make_casting_demo.py`.
